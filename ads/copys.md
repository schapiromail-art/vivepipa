# Copys de anúncio — Pele de Filtro

Nove ângulos. Os três primeiros vieram de `pesquisa/voz-do-cliente.md` e usam frases
literais do nicho — **são os que devem ir ao ar primeiro.** Os seis seguintes eram os
originais e continuam válidos, um degrau abaixo em intensidade.

Suba hoje os **ângulos A e B**, cada um em seu próprio conjunto isolado (estrutura
Testadora 1-1-1 da palestra, R$ 16,65/dia cada). Os outros entram nos dias seguintes,
conforme os criativos ficarem prontos.

## ⚠️ A regra que decide se o anúncio roda

Da auditoria de políticas: **o que derruba anúncio de beleza quase nunca é o produto — é
o enquadramento.** Três coisas reprovam:

1. Apresentar um traço normal como defeito a corrigir
2. **Abrir com a insegurança de quem está vendo**
3. Prometer resultado garantido

O enquadramento seguro é **o produto e a experiência de usá-lo**, nunca a carência da
pessoa.

Isso reprova a headline mais forte que a pesquisa encontrou. *“Sua pele parece mais velha
com maquiagem do que sem”* é exatamente o item 2 — abre falando da aparência dela, em
tom negativo. Converte, e é justamente por isso que o Meta derruba.

**A divisão que resolve:**

| | Fala de | Exemplo |
|---|---|---|
| **Anúncio** | a **base** | “A base craquela porque a camada é grossa demais” |
| **Página** | a **experiência dela** | “Sua pele parece mais velha com maquiagem do que sem” |

O anúncio precisa passar pela revisão automática do Meta; a página, não. Ela chega na
página já tendo clicado — a identificação forte funciona ali, com risco muito menor.
Por isso a headline da pesquisa continua sendo o hero da landing e **não** aparece em
nenhum anúncio.

Sinal prático: se a frase tem “você” + adjetivo sobre a aparência dela, reescreva com a
base como sujeito.

---

## A fórmula: PRSA

Todo criativo segue a estrutura da palestra:

| | | |
|---|---|---|
| **P** | Problema | o que a base faz de errado |
| **R** | Rota | o caminho errado que ela está seguindo hoje |
| **S** | Solução | o mecanismo, em uma frase |
| **A** | Ação | o pedido, com preço e garantia |

---

Regras que valem para todos:
- Primeira linha é 100% do trabalho. Ela aparece antes do "ver mais".
- Falar **com ela**, não sobre ela. "A sua base", não "as bases".
- Nunca prometer resultado de pele ou rejuvenescimento. A promessa é sobre **a maquiagem**,
  não sobre o rosto. Isso mantém o anúncio fora da política de saúde e beleza do Meta.
- Sem "antes e depois" da pele. Se usar comparação, que seja da **maquiagem aplicada**.

---

## Ângulo A — A camada grossa ★★ principal

*PRSA. Sujeito da frase: a base. Nunca ela.*

**Texto principal**
> **[P]** A base craquela ao longo do dia e se acumula dentro do bigode chinês.
>
> **[R]** A correção que todo mundo tenta é trocar de base. Já foram três, e nenhuma
> resolveu — porque nenhuma delas ia resolver.
>
> **[S]** O problema não é a fórmula, é a espessura. Depois dos 35 a pele retém menos
> água, e camada grossa não acompanha o movimento do rosto: ela trava dentro da linha e
> quebra ali. A mesma base, em três camadas finíssimas com 40 segundos entre elas, não
> faz isso — nenhuma tem espessura suficiente para quebrar.
>
> **[A]** O protocolo completo leva 10 minutos e funciona com a base que já está na sua
> gaveta. Pele de Filtro, R$ 37, acesso imediato, 7 dias de garantia.

**Título:** A base não é o problema. A espessura é.
**Descrição:** Protocolo de 10 minutos — R$ 37
**CTA:** Saiba mais

---

## Ângulo B — A base que faz bolinhas ★★ ângulo virgem

*Quase ninguém separa talhada de craquelada. Quem sofre disso procura e não encontra
nada específico.*

**Texto principal**
> **[P]** A base faz bolinhas na hora de passar. Você espalha, ela junta, e o rosto fica
> com textura irregular.
>
> **[R]** A reação natural é esfregar para desfazer — e isso tira a maquiagem e leva o
> hidratante junto. Depois vem a conclusão errada: “essa base é ruim”.
>
> **[S]** Isso não é base craquelada, é base **talhada**, e a causa é outra:
> incompatibilidade de textura. Hidratante em gel embaixo de base de silicone separa na
> hora. Água com água, silicone com silicone — e a bolinha some.
>
> **[A]** A correção leva dez segundos e está no Pele de Filtro, junto com o protocolo
> completo. R$ 37, acesso imediato.

**Título:** Base que faz bolinhas tem nome: talhada
**Descrição:** E a causa não é a base — R$ 37
**CTA:** Saiba mais

---

## Ângulo C — O bigode chinês ★ específico

**Texto principal**
> **[P]** No fim do dia o bigode chinês fica mais marcado com base do que sem.
>
> **[R]** A tentativa óbvia é passar menos produto ali. Não resolve — e ainda deixa a
> área desigual em relação ao resto do rosto.
>
> **[S]** É o ponto que mais se move, então é o primeiro a entregar camada grossa. A
> correção é aplicar mais fino no rosto **todo** e pressionar a esponja em vez de
> arrastar: arrastar é o que empurra o produto para dentro do vinco.
>
> **[A]** Dez minutos, com a base que você já tem. Pele de Filtro, R$ 37.

**Título:** Por que o bigode chinês fica mais marcado com base
**Descrição:** Não é passar menos ali — R$ 37
**CTA:** Saiba mais

---

## Ângulo 1 — O craquelado (dor direta) ★ principal

**Texto principal**
> Passa a base às 7h e às 11h já craquelou embaixo do olho.
>
> Não é a sua base. É a espessura.
>
> Depois dos 35 a pele retém menos água, e uma camada grossa de base não acompanha o
> movimento do rosto: ela se acumula dentro da linha de expressão e quebra ali.
>
> A mesma base, aplicada em três camadas finíssimas com 40 segundos entre elas, não racha.
> Nenhuma camada tem espessura suficiente para quebrar.
>
> É o Método das 3 Camadas Finas. São 10 minutos e você faz com o que já tem na gaveta.
>
> Protocolo completo + 4 bônus por R$ 37. Teste amanhã de manhã. Se craquelar do mesmo
> jeito, você pede o dinheiro de volta em 7 dias.

**Título:** A sua base não é o problema. A espessura é.
**Descrição:** Protocolo de 10 minutos — R$ 37
**CTA:** Comprar agora

---

## Ângulo 2 — O erro contraintuitivo ★ principal

**Texto principal**
> O erro nº 1 depois dos 35 é passar base do mesmo jeito que você passava aos 25.
>
> Você aprendeu a cobrir. E cobrir é exatamente o que faz a maquiagem marcar a linha
> embaixo do olho e te deixar com cara de cansada.
>
> A correção é o contrário do que todo mundo faz: menos produto, mais camadas, tempo de
> espera entre elas. E quase nada de pó na área dos olhos — pó em excesso é a causa número
> um do vinco.
>
> Está tudo no Pele de Filtro: o protocolo de 10 minutos, passo a passo, com o tempo de
> cada camada. R$ 37, acesso imediato, 7 dias de garantia.

**Título:** Menos base cobre mais. Sério.
**Descrição:** O protocolo de 10 minutos — R$ 37
**CTA:** Saiba mais

---

## Ângulo 3 — A foto (momento de dor) ★ principal

**Texto principal**
> Você se arrumou, se olhou no espelho e estava tudo certo.
>
> Aí viu a foto do aniversário.
>
> A base craquelada, a linha embaixo do olho marcada, o rosto parecendo pesado. E você
> pensando: eu não estava assim no espelho.
>
> Estava. A luz do banheiro perdoa camada grossa. A câmera não.
>
> O que resolve não é base nova — é aplicar a que você já tem em três camadas finíssimas,
> com tempo de secagem entre elas. Dez minutos.
>
> Pele de Filtro: protocolo completo + 4 bônus, R$ 37.

**Título:** Bem no espelho, horrível na foto?
**Descrição:** O que a câmera mostra e o espelho esconde — R$ 37
**CTA:** Comprar agora

---

## Ângulo 4 — Objeção de dinheiro (reserva)

**Texto principal**
> Quantas bases você já comprou tentando resolver o craquelado?
>
> Se a resposta é mais de duas, o problema não está no produto. Nenhuma delas ia funcionar,
> porque todas estavam sendo aplicadas do jeito que faz qualquer base rachar numa pele
> madura: camada única e grossa.
>
> O Pele de Filtro não pede que você compre nada. O protocolo funciona com a base que está
> na sua gaveta agora — inclusive aquela que você achou que era ruim.
>
> R$ 37. Menos do que a última base que não resolveu.

**Título:** Antes de comprar outra base, leia isto
**Descrição:** Funciona com a que você já tem — R$ 37
**CTA:** Saiba mais

---

## Ângulo 5 — A base que fica alaranjada ★ principal

**Texto principal**
> A base parecia certa na loja e no seu rosto ficou alaranjada.
>
> Isso tem nome: oxidação. A base reage com o óleo natural da pele e escurece em cerca de
> 10 minutos — por isso o tom só fica errado depois que você já saiu de casa.
>
> Tem correção, e ela é simples: escolher o tom um degrau mais claro do que parece certo no
> primeiro minuto, e usar primer só nas zonas oleosas para reduzir o contato.
>
> Essa é uma das correções do Pele de Filtro. As outras nove estão no protocolo de 10
> minutos — R$ 37, acesso imediato.

**Título:** Por que a sua base fica laranja depois de 10 minutos
**Descrição:** Tem correção e não é trocar de base — R$ 37
**CTA:** Saiba mais

---

## Ângulo 6 — Evento com data (reserva, para escalar)

**Texto principal**
> Casamento no sábado e você já sabe o que vai acontecer: às 22h a maquiagem estará
> craquelada e você fugindo das fotos.
>
> Dá para mudar isso ainda esta semana. Não com produto novo — com a ordem certa de
> aplicação: três camadas finíssimas, 40 segundos entre elas, quase nada de pó embaixo do
> olho.
>
> Dez minutos para aprender, e vale para todos os eventos depois desse.
>
> Pele de Filtro — R$ 37, acesso imediato.

**Título:** Para a maquiagem durar até o fim da festa
**Descrição:** Protocolo de 10 minutos — R$ 37
**CTA:** Comprar agora

---

## Vocabulário obrigatório

Nunca escreva **“ruga”** nem **“envelhecida”** — o nicho fala em “linha de expressão” e
“pele madura”, e as duas palavras proibidas soam como acusação. Diga **“bigode chinês”**,
não “sulco nasogeniano”. Glossário completo em `pesquisa/voz-do-cliente.md`.

---

## Variações de título para teste rápido

Se o CTR ficar baixo mas a copy parecer boa, troque só o título antes de matar o criativo:

- A sua base não é o problema. A espessura é.
- Por que a maquiagem craquela depois dos 35
- Três camadas finas em vez de uma grossa
- O pó é o que está marcando o seu olho
- Bem no espelho, horrível na foto?
- Menos base cobre mais
- Sua base faz bolinhas? Não é a base
- Fica bonito de manhã e some ao longo do dia
- A camada grossa é que trava dentro da linha

*(Nenhum destes fala da aparência dela — todos têm a base como sujeito. Ver a regra de
política no topo do arquivo.)*
